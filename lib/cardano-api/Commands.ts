...
export const Commands = {
  ...
  getBalance: async (): Promise<number> => {
    const utxosHexPromise: Promise<TransactionUnspentOutput[]> =
      CardanoAPIObject.baseCommands.getUtxos();
    const utxos: Promise<TransactionUnspentOutput[]> = utxosHexPromise.then(
      (utxosHex: TransactionUnspentOutput[]) =>
        utxosHex.map((value: TransactionUnspentOutput) =>
          CardanoAPIObject.serializationLib.TransactionUnspentOutput.from_bytes(
            Buffer.from(value.to_hex(), "hex")
          )
        )
    );
    const totalBalance = utxos.then((utxosHex: TransactionUnspentOutput[]) =>
      utxosHex.reduce((sum: number, utxo: TransactionUnspentOutput) => {
